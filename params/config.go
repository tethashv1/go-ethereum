...
type ChainConfig struct {
	...
	TEthashV1Block      *big.Int `json:"tethashv1Block,omitempty"`      // TEthashV1 switch block (nil = no fork, 0 = already activated)
...
func (c *ChainConfig) String() string {
	...
	return fmt.Sprintf("{ChainID: %v Homestead: %v DAO: %v DAOSupport: %v EIP150: %v EIP155: %v EIP158: %v Byzantium: %v Constantinople: %v TEthashV1: %v Engine: %v}",
		c.ChainID,
		c.HomesteadBlock,
		c.DAOForkBlock,
		c.DAOForkSupport,
...
// IsTEthashV1 returns whether num represents a block number after the TEthashV1 fork
func (c *ChainConfig) IsTEthashV1(num *big.Int) bool {
	return isForked(c.TEthashV1Block, num)
}
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, head *big.Int) *ConfigCompatError {
	...
	if isForkIncompatible(c.TEthashV1Block, newcfg.TEthashV1Block, head) {
		return newCompatError("TEthashV1 fork block", c.TEthashV1Block, newcfg.TEthashV1Block)
	}
	return nil
}
